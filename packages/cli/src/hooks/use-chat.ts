...
// this is basically a replication of ai sdk's useChat with same features
export function useChat(sessionId: string, initialMessages: Message[]) {
    ...
    for await (const { data } of stream) {
      ...
      switch (event.type) {
        case "reasoning-delta": {
          console.log("GOT REASONING DELTA:", event.text);
        ...
        case "text-delta": {
          const last = parts[parts.at.length - 1];
  ...
  // auto resume when the convo ends with a user message that has no reply
  ...
  const submit = useCallback(async (
    ...
    // show partial answer before sending next message
